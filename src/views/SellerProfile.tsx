    ...
    try {
      ...
      if (sellerData) {
        ...
        if (providerData) {
          ...
          setCoverUrl(null); // Providers ainda não tem cover no banco, usamos placeholder
          setWhatsapp('');
          setInstagram('');
          setIsVerified(true);
          setViews(0);
  ...
  return (
    <div className="min-h-screen bg-neutral-50 pb-24">
      ...
      <div className="relative h-48 md:h-64 w-full bg-neutral-200">
        {isOwner && profileType === 'seller' ? (
          <AvatarUploader
            currentUrl={coverUrl}
            fallbackUrl={`https://picsum.photos/seed/${coverSeed}/1200/400`}
            onUploadSuccess={(url) => {
              setCoverUrl(url);
              supabase.from('sellers').update({ cover_url: url }).eq('id', profileId).then();
            }}
            uid={userId || ''}
            folder="covers"
            size="cover"
            ...
            {isOwner ? (
              <div className="z-10 relative">
                <AvatarUploader
                  ...
                  fallbackUrl={`https://picsum.photos/seed/${avatarSeed}profile/200/200`}
                  onUploadSuccess={(url) => {
                    setAvatarUrl(url);
                    if (profileType === 'seller') {
                      supabase.from('sellers').update({ avatar_url: url }).eq('id', profileId).then();
                    } else if (profileType === 'provider') {
                      supabase.from('service_providers').update({ avatar_url: url }).eq('id', profileId).then();
                    }
                  }}
                  uid={userId || ''}
                  folder="stores"
        ...
        <div className="mb-8 p-4 rounded-2xl bg-orange-50 border border-orange-100 flex items-center justify-between">
          <div className="flex items-center gap-3 overflow-hidden">
            ...
            <div className="truncate">
              <p className="text-xs font-bold text-orange-600/70 uppercase tracking-widest">Link de Acesso</p>
